...
goog.provide('Blockly.TestProcedures');
 ...
 */
Blockly.TestProcedures.NAME_TYPE = 'PROCEDURE-TESTS';
...
Blockly.TestProcedures.allProcedures = function(root) {
  var blocks = VisualBlocks._workspaces.appWorkspace.getAllBlocks();;
  ...
  proceduresNoReturn.sort(Blockly.TestProcedures.procTupleComparator_);
  proceduresReturn.sort(Blockly.TestProcedures.procTupleComparator_);
 ...
 * @private
 */
Blockly.TestProcedures.procTupleComparator_ = function(ta, tb) {
 ...
 */
Blockly.TestProcedures.findLegalName = function(name, block) {
  if (block.isInFlyout) {
    // Flyouts can have multiple procedures called 'do something'.
    return name;
  }
  while (!Blockly.TestProcedures.isLegalName(name, block.workspace, block)) {
 ...
 * @this {!Blockly.Field}
 */
Blockly.TestProcedures.rename = function(text) {
  ...
  // Rename any callers.
  var blocks = this.sourceBlock_.VisualBlocks._workspaces.appWorkspace.getAllBlocks();;
  for (var i = 0; i < blocks.length; i++) {
 ...
 */
Blockly.TestProcedures.flyoutCategory = function(workspace) {
  var xmlList = [];
  ...
  var tuple = Blockly.TestProcedures.allProcedures(workspace);
 ...
 */
Blockly.TestProcedures.disposeCallers = function(name, workspace) {
  var callers = Blockly.TestProcedures.getCallers(name, workspace);
 ...
 */
Blockly.TestProcedures.mutateCallers = function(name, workspace,
                                            paramNames, paramIds) {
  var callers = Blockly.TestProcedures.getCallers(name, workspace);
  for (var i = 0; i < callers.length; i++) {
